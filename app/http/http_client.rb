  ...
  module Http
    ...
    class HttpClient < Integration::Http::HttpClient
      ...
      def getURLResponse(url)
        ...
        res = nil
        BLog.logger.info("HTTP Get #{url} on #{Dispatch::Queue.current}") if BLog::INFO
        resp = AFMotion::HTTP.get(url) do |result|
          ...
          @queue.async do
            @entry.wait
            if result.failure?
              BLog.logger.info("HTTP failure on #{url} #{result.inspect}") if BLog::INFO
              @result = HttpResponse.new(result)
            else
              message = result.body.to_s
              BLog.logger.info("HTTP Got #{message}") if BLog::INFO
              @result = HttpResponse.new(result)
            end
            @sem.signal
          end
      ...
      def postURLResponse(url, params)
        ...
        #return Integration::Http::HttpEntity.new(MockWrap.new(mock)) if /discover\?lon/ =~ url
        res = nil
        BLog.logger.info("HTTP Post #{url} on #{Dispatch::Queue.current}") if BLog::INFO
        BLog.logger.info("Params #{params.inspect}") if BLog::INFO
        ...
        resp = AFMotion::HTTP.post(url, afParams) do |result|
          ...
          @queue.async do
            ...
            if result.failure?
              ...
              message = result.body.to_s
              BLog.logger.info("HTTP Post Got #{message}") if BLog::INFO
              @result = HttpResponse.new(result)
            end
            @sem.signal
          end
